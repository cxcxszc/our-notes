...
export function useNotes(pairId: string | undefined) {
  ...
  const markNoteViewed = useCallback(
    async (noteId: string) => {
      const note = notes.find((n) => n.id === noteId);
      if (!note) return;

      const updates: Record<string, unknown> = { lastViewedAt: serverTimestamp() };
      if (!note.isRead) {
        updates.isRead = true;
  ...
  return { notes, loading, addNote, updateNote, deleteNote, togglePin, toggleReaction, markNoteViewed };
}
